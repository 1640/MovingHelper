...
public struct TaskLoader {
  ...
  */
  ...
  private static func tasksFromData(data: NSData) -> [Task]? {
    do {
    let arrayOfTaskDictionaries = try NSJSONSerialization.JSONObjectWithData(data, options: [])
    return Task.tasksFromArrayOfJSONDictionaries(arrayOfTaskDictionaries as! [NSDictionary])
